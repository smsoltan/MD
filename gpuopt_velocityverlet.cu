//nvcc -o gpuopt_velocityverlet gpuopt_velocityverlet.cu
#include <stdio.h>
#include <stdlib.h>
#include <math.h>
#include <time.h>
...
//modification: now it also finishes summing K and V (not this, the next)
__device__ void __gpu_sync(int goalVal, volatile int *Arrayin,
                                        volatile int *Arrayout,
                                        double *vb/*,
                           float mK, float mV,
                           volatile float *Kb, volatile float *Vb,
                           float *K, float *V*/)
{
...
//modification: can break the loop
__device__ void __gpu_small_sync(int goalVal, int oldGoalVal,
                           volatile int *Arrayin, volatile int *Arrayout,
                           float mK, float mV, double mv,
         volatile float *Kb, volatile float *Vb, volatile double *vb,
                           float *K, float *V, volatile bool *loop)
{
 // thread ID in a block
 int tid_in_block = threadIdx.x * blockDim.y + threadIdx.y;
 int nBlockNum = gridDim.x * gridDim.y;
 ...
 if (tid_in_block == 0)
 {
  Kb[bid] = mK; Vb[bid] = mV;
  if (!(*loop)) vb[bid] = 0;
  double pause = vb[bid]; vb[bid] = pause > mv ? pause : mv;
  Arrayin[bid] = goalVal;
 }
 if (bid == 1)
 {
  ...
  __shared__ float2 cache[blocksPerGrid];
  __shared__ double control[blocksPerGrid];
  if (tid_in_block < nBlockNum) {cache[tid_in_block].x = Kb[tid_in_block];
                                 cache[tid_in_block].y = Vb[tid_in_block];
                                 control[tid_in_block] = vb[tid_in_block];}
  int i = nBlockNum/2;
  __syncthreads();
  while(i != 0)
  {
...
double temp = control[tid_in_block], temp2 = control[tid_in_block+i];
control[tid_in_block] = (temp > temp2) ? temp : temp2;}
   __syncthreads();
   i /= 2;
  }
  if (tid_in_block == 0) {K[goalVal] = cache[0].x; V[goalVal] = cache[0].y;
*loop = (goalVal + 1 < oldGoalVal
         + static_cast<int>((RM-RC)/(4*control[0]*step_dev)));}
  if (tid_in_block < nBlockNum)
  {
   Arrayout[tid_in_block] = goalVal;
  }
   ...
   for(int ii = blockIdx.x + 1; ii < blockIdx.x+gridDim.x; ++ii)
   {
      ...
      for(int j = 0; j < N_dev; ++j)
         if(j != tid)
         {
            ...
            if (x > RM) x-= S_dev; else if (x < -RM) x+= S_dev;
            if (y > RM) y-= S_dev; else if (y < -RM) y+= S_dev;
            if (z > RM) z-= S_dev; else if (z < -RM) z+= S_dev;
            double r2 = x*x + y*y + z*z;
            if (r2 < RM*RM)
            {
             verletList[*neighbours] = j;
             (*neighbours)++;
             if (r2 < RC*RC)
             {
                double R = pow(SIGMA*SIGMA/r2, 3);
...
__global__ void VelocityVerlet
                (double *r, double *v, double *a, 
                 double *rn, float *Kb, float *Vb, double *vb,
                 float *K, float *V,
                 int *Arrayin, int *Arrayout, volatile bool *loop)
{
   int tid = threadIdx.x + blockIdx.x * blockDim.x;
   int cacheIndex = threadIdx.x;
...
//   else if (cacheIndex == 1) Arrayout[blockIdx.x] = -1;
   __gpu_sync(-1, Arrayin, Arrayout, vb);
...
//   bool whichTexture = true;
  ...
  // int *verletList = (int*) malloc((N_dev-1)*sizeof(int));
   int verletList[threadsPerBlock*2-1];
   volatile int ot;
   int neighbours;
for (int it = 0; it < l_dev; ++it)
{
//calculating a and constructing verlet list
   double ax_new = 0, ay_new = 0, az_new = 0; 
   if (*loop) cache2[cacheIndex] = acalc(&ax_new, &ay_new, &az_new,
                              x_load, y_load, z_load, r,
                              verletList, neighbours);
   else {ot = it;
         cache2[cacheIndex] = verletListConstruct(&ax_new, &ay_new, &az_new,
                              x_load, y_load, z_load, r,
                              verletList, &neighbours);
//if(cacheIndex == 0) printf("%d,%d\n", blockIdx.x, ot);
//         __gpu_sync(it+1, Arrayin, Arrayout);
        }
//   __gpu_sync(it+1, Arrayin, Arrayout);
//calculating v
...
//measurement of temperature
   ...
   cache[cacheIndex] = static_cast<float>(v);
   cache3[cacheIndex] = sqrt(v);
   __syncthreads();
   int i = blockDim.x/2;
   while (i != 0)
   {
 ...
 double temp1 = cache3[cacheIndex], temp2 = cache3[cacheIndex+i];
 cache3[cacheIndex] = (temp1 > temp2) ? temp1 : temp2;
      }
      __syncthreads();
      i /= 2;
   }
...
//calculating r
   ...
   __gpu_small_sync(it, ot, Arrayin, Arrayout,
              cache[0]/2, cache2[0]/2, cache3[0],
              Kb, Vb, vb, K, V, loop);
}
}
...
//                                  dev_rn, 3*N*sizeof(float)));
   ...
   bool *dev_loop;
   HANDLE_ERROR( cudaMalloc( (void**)&dev_loop,
                              sizeof(bool) ) );
   bool loop = false;
   HANDLE_ERROR( cudaMemcpy( dev_loop, &loop, sizeof(bool),
                              cudaMemcpyHostToDevice ) );
...
//   cudaPrintfInit();
   ...
   VelocityVerlet<<<blocksPerGrid, threadsPerBlock, 3*N*sizeof(double)>>>
                (dev_r, dev_v, dev_a, dev_rn,
                 dev_Kb, dev_Vb, dev_vb, dev_K, dev_V,
                 dev_Arrayin, dev_Arrayout, dev_loop);
